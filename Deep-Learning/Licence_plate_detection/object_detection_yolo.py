...
#LP_extracted=cv.imread("Hawk_Eye/test.png")
...
def drawPred(classId, conf, left, top, right, bottom,frame):
    ...
    cv.imwrite(directory+"Licence_Plate_extracted.jpg",LP_extracted)     	#extracting the licence plate
...
def LP_detection():
	...
	if (not args.image):
	    vid_writer = cv.VideoWriter(directory+outputFile, cv.VideoWriter_fourcc('M','J','P','G'), 30, (round(cap.get(cv.CAP_PROP_FRAME_WIDTH)),round(cap.get(cv.CAP_PROP_FRAME_HEIGHT))))
	...
	while cv.waitKey(1) < 0:
	    ...
	    if (args.image):
	    	...
	    	vid_writer.write(directory+frame.astype(np.uint8))
	return(LP_extracted)
